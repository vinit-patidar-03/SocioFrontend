...
const CreatedPostCard = (props) => {
  ...
  useEffect(() => {
    ...
    setStats({ comments: post?.comments?.length, likes: post?.likes?.length })
  }, [post?.comments?.length, post.likes, user])
    ...
    <>{user &&
      <div className='p-3 bg-white mx-auto my-3 sm:rounded-lg sm:w-[500px] w-full sm:shadow-[0_0px_3px_0px_rgba(0,0,0,0.3)]'>
        ...
        <div className='m-auto bg-slate-100 rounded-lg w-[95%]'>
          <img src={post.photo} className='m-auto w-full h-[300px] object-contain' alt="logo" loading='lazy' />
        </div>
        ...
        <div className='m-2 w-[95%] flex items-center'>
          ...
          <button className='ml-2' onClick={() => {
            ...
            <IoMdSend className='text-orange-600 text-3xl hover:text-orange-400 transition-all hover:transition-all' />
          </button>
        </div>
